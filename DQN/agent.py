...
class DQNAgent:
    def __init__(self, env, render, config_info):
        ...
        self.start_eps = config["training"]["start_epsilon"]
        self.final_eps = config["training"]["final_epsilon"]
        self.decay_eps = config["training"]["decay_epsilon"]
    ...
    def play_step(self, epsilon=0.1):
        reward_signal = None

        # Agent selects actions following an epsilon-greedy behaviour policy
        if np.random.random() < epsilon:
            # Random action choice = exploration
    ...
    def train(self):
        ...
        episode_num = 0
        eps = self.start_eps  # start epsilon to explore while filling the buffer
        ...
        for t in range(int(self.max_timesteps)):
            ...
            reward_signal = self.play_step(eps)
            ...
            if len(self.replay_buffer) < self.replay_size:
                ...
                eps = np.maximum(self.final_eps, self.start_eps - (t / self.decay_eps))
    ...
    def eval_policy(self, eval_episodes=10):
        ...
        for _ in range(eval_episodes):
            ...
            while reward_signal is None:
                reward_signal = self.play_step(epsilon)
